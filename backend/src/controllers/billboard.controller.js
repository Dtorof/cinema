...
export const createResponse = async  (req,res) => {
    ...
    let { room_id, movie_id, start_date, end_date  } = req.body
    const getAge = end_date => Math.floor((new Date() - new Date(end_date).getTime()) / 3.15576e+10)
    const age =getAge(end_date)
    const state = ''
    age > 1 ? state='Disponible' : state='No Disponible'
